...
class OnboardingBloc extends HydratedBloc<OnboardingEvent, OnboardingState> {
  ...
  Map<String, dynamic>? toJson(OnboardingState state) {
    ...
    if (state is DocumentUploaded)   return {'type': 'DocumentUploaded'};
  ...
  Future<void> _onSaveConsent(SaveConsentEvent event, Emitter<OnboardingState> emit) async {
    emit(const OnboardingLoading());
    await Future.delayed(const Duration(milliseconds: 300));
    emit(const ConsentSaved());
  }

  Future<void> _onSavePersonalInfo(SavePersonalInfoEvent event, Emitter<OnboardingState> emit) async {
    emit(const OnboardingLoading());
    await Future.delayed(const Duration(milliseconds: 300));
    emit(const PersonalInfoSaved());
  }

  Future<void> _onVerifyBvn(VerifyBvnEvent event, Emitter<OnboardingState> emit) async {
    emit(const OnboardingLoading(message: 'Verifying BVN with NIBSS…'));
    final result = await verifyBvn(VerifyBvnParams(bvn: event.bvn));
    result.fold(
      (failure) => emit(OnboardingError(message: failure.message, code: failure.code)),
      (_) => emit(const BvnVerified()),
    );
  }

  Future<void> _onUploadDocument(UploadDocumentEvent event, Emitter<OnboardingState> emit) async {
  ...
  Future<void> _onUploadFaceCapture(UploadFaceCaptureEvent event, Emitter<OnboardingState> emit) async {
    emit(const OnboardingLoading(message: 'Analysing biometrics…'));
    final result = await uploadFaceCapture(UploadFaceCaptureParams(
  ...
  Future<void> _onLoadSavedProgress(LoadSavedProgressEvent event, Emitter<OnboardingState> emit) async {
  ...
  Future<void> _onReset(ResetOnboardingEvent event, Emitter<OnboardingState> emit) async {
